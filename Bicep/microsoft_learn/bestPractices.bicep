...
@description('Type of Environment. Allowed values are Production and Test')
@ allowed([
  'Production'
  'Test'
])
...
@description('Unique suffix to add to resource names that must be globally unique')
@maxLength(13)
...
resource contributorRoleAssignment 'Microsoft.Authorization/roleAssignments@2020-04-01-preview' = {
  ...
  // This will return the same GUID every time the template is deployed to the same resource group.
